...
def spatial_attention(input_tensor, **kwargs):
    with tf.variable_scope('spatial_attention'):
        at = slim.conv2d(input_tensor, 1, kernel_size=3, weights_initializer=tf.truncated_normal_initializer(stddev=0.01))
        gating_fn = lambda x: tf.nn.relu6(x + 3) * 0.16667
        at = gating_fn(at)
        return input_tensor * at


def gdconv_to_1x1(input_tensor, default_size=7, **kwargs):
    with tf.variable_scope('gdconv'):
        h, w = input_tensor.shape.as_list()[1:3]
        if h is not None and w == h:
            k = [h, h]
        else:
            k = [default_size, default_size]
        net = slim.separable_conv2d(inputs=input_tensor, num_outputs=None, kernel_size=k, stride=1,
                                    depth_multiplier=1.0, activation_fn=None, padding='VALID')
        return net


def flatten_to_1x1(net, default_size=7, **kwargs):
    with tf.variable_scope('gdconv'):
        net = slim.conv2d(inputs=net, num_outputs=224, kernel_size=[1, 1], stride=2, activation_fn=None, padding='VALID')
        net = tf.reshape(net, [-1, 1, 1, net.get_shape()[1] * net.get_shape()[2] * net.get_shape()[3]])
        return net
...
V3_LARGE_FACE_OP1S1_GDCONV = dict(
    defaults=dict(DEFAULTS),
    spec=([
        # stage 1
        op(slim.conv2d, stride=1, num_outputs=16, kernel_size=(3, 3),
        ...
        op(slim.conv2d, stride=1, kernel_size=[1, 1], num_outputs=960,
           activation_fn=hard_swish),
        op(gdconv_to_1x1, default_size=7, stride=1, padding='VALID'),
        # op(slim.conv2d, stride=1, kernel_size=[1, 1], num_outputs=1280,
        #    normalizer_fn=None, activation_fn=hard_swish)
    ]))
...
V3_LARGE_FACE = dict(
    ...
    spec=([
        ...
        mbv3_op_se(ef=6, n=112, k=3, s=1, act=hard_swish),
        mbv3_op_se(ef=6, n=112, k=3, s=1, act=hard_swish),
        mbv3_op_se(ef=6, n=160, k=3, s=2, act=hard_swish),
        mbv3_op_se(ef=6, n=160, k=3, s=1, act=hard_swish),
        mbv3_op_se(ef=6, n=160, k=3, s=1, act=hard_swish),
        op(slim.conv2d, stride=1, kernel_size=[1, 1], num_outputs=960,
           activation_fn=hard_swish),
        op(reduce_to_1x1, default_size=7, stride=1, padding='VALID'),
...
V3_LARGE_MINIMALISTIC_FACE = dict(
    defaults=dict(DEFAULTS),
    spec=([
        # stage 1
        op(slim.conv2d, stride=2, num_outputs=16, kernel_size=(3, 3)),
...
V3_LARGE_MINIMALISTIC_FACE_RELU6 = dict(
    defaults=dict(DEFAULTS),
    spec=([
        # stage 1
        op(slim.conv2d, stride=2, num_outputs=16, kernel_size=(3, 3), activation_fn=tf.nn.relu6),
        mbv3_op(ef=1, n=16, k=3, act=tf.nn.relu6),
        mbv3_op(ef=4, n=24, k=3, s=2, act=tf.nn.relu6),
        mbv3_op(ef=3, n=24, k=3, s=1, act=tf.nn.relu6),
        mbv3_op(ef=3, n=40, k=3, s=1, act=tf.nn.relu6),
        mbv3_op(ef=3, n=40, k=3, s=1, act=tf.nn.relu6),
        mbv3_op(ef=3, n=40, k=3, s=1, act=tf.nn.relu6),
        mbv3_op(ef=6, n=80, k=3, s=2, act=tf.nn.relu6),
        mbv3_op(ef=2.5, n=80, k=3, s=1, act=tf.nn.relu6),
        mbv3_op(ef=184 / 80., n=80, k=3, s=1, act=tf.nn.relu6),
        mbv3_op(ef=184 / 80., n=80, k=3, s=1, act=tf.nn.relu6),
        mbv3_op(ef=6, n=112, k=3, s=1, act=tf.nn.relu6),
        mbv3_op(ef=6, n=112, k=3, s=1, act=tf.nn.relu6),
        mbv3_op(ef=6, n=160, k=3, s=2, act=tf.nn.relu6),
        mbv3_op(ef=6, n=160, k=3, s=1, act=tf.nn.relu6),
        mbv3_op(ef=6, n=160, k=3, s=1, act=tf.nn.relu6),
        op(slim.conv2d, stride=1, kernel_size=[1, 1], num_outputs=960, activation_fn=tf.nn.relu6),
        op(reduce_to_1x1, default_size=7, stride=1, padding='VALID'),
        op(slim.conv2d,
           stride=1,
           kernel_size=[1, 1],
           num_outputs=512,
           normalizer_fn=None, activation_fn=tf.nn.relu6)
    ]))

V3_LARGE_MINIMALISTIC_FACE_OP1S1 = dict(
    defaults=dict(DEFAULTS),
    spec=([
        # stage 1
        op(slim.conv2d, stride=1, num_outputs=16, kernel_size=(3, 3)),
        mbv3_op(ef=1, n=16, k=3, s=1),
        mbv3_op(ef=4, n=24, k=3, s=2),
        mbv3_op(ef=3, n=24, k=3, s=1),
...
V3_LARGE_MINIMALISTIC_FACE_OP1S1_FC2 = dict(
    defaults=dict(DEFAULTS),
    spec=([
        # stage 1
        op(slim.conv2d, stride=1, num_outputs=16, kernel_size=(3, 3)),
...
V3_LARGE_MINIMALISTIC_FACE_OP1S1_GDCONV = dict(
    defaults=dict(DEFAULTS),
    spec=([
        # stage 1
        op(slim.conv2d, stride=1, num_outputs=16, kernel_size=(3, 3)),
...
V3_LARGE_MINIMALISTIC_FACE_OP3S2 = dict(
    defaults=dict(DEFAULTS),
    spec=([
        # stage 1
        op(slim.conv2d, stride=2, num_outputs=16, kernel_size=(3, 3)),
        mbv3_op(ef=1, n=16, k=3),
        mbv3_op(ef=4, n=24, k=3, s=1),
        mbv3_op(ef=3, n=24, k=3, s=1),
        mbv3_op(ef=3, n=40, k=3, s=2),
        mbv3_op(ef=3, n=40, k=3, s=1),
        mbv3_op(ef=3, n=40, k=3, s=1),
...
V3_LARGE_MINIMALISTIC_FACE_HSWISH = dict(
    defaults=dict(DEFAULTS),
    spec=([
        # stage 1
        op(slim.conv2d, stride=2, num_outputs=16, kernel_size=(3, 3)),
        ...
        mbv3_op(ef=184 / 80., n=80, k=3, s=1, act=hard_swish),
        mbv3_op(ef=184 / 80., n=80, k=3, s=1, act=hard_swish),
        mbv3_op(ef=6, n=112, k=3, s=1, act=hard_swish),
        mbv3_op(ef=6, n=112, k=3, s=1, act=hard_swish),
        mbv3_op(ef=6, n=160, k=3, s=2, act=hard_swish),
        mbv3_op(ef=6, n=160, k=3, s=1, act=hard_swish),
        mbv3_op(ef=6, n=160, k=3, s=1, act=hard_swish),
        op(slim.conv2d, stride=1, kernel_size=[1, 1], num_outputs=960),
        op(reduce_to_1x1, default_size=7, stride=1, padding='VALID'),
        op(slim.conv2d,
...
V3_LARGE_MINIMALISTIC_FACE_GDCONV = dict(
    defaults=dict(DEFAULTS),
    spec=([
        # stage 1
        op(slim.conv2d, stride=2, num_outputs=16, kernel_size=(3, 3)),
...
V3_LARGE_MINIMALISTIC_FACE_FLATTEN = dict(
    defaults=dict(DEFAULTS),
    spec=([
        # stage 1
        op(slim.conv2d, stride=2, num_outputs=16, kernel_size=(3, 3)),
        ...
        op(slim.conv2d, stride=1, kernel_size=[1, 1], num_outputs=960),
        # op(reduce_to_1x1, default_size=7, stride=1, padding='VALID'),
        op(flatten_to_1x1, default_size=7, stride=1, padding='VALID'),
        op(slim.conv2d,
           stride=1,
           kernel_size=[1, 1],
           num_outputs=256,
...
V3_LARGE_MINIMALISTIC_FACE_RS = dict(
    defaults=dict(DEFAULTS),
    spec=([
        # stage 1
        op(slim.conv2d, stride=2, num_outputs=32, kernel_size=(3, 3)), # pre
        mbv3_op(ef=1, n=24, k=3), # 0
        mbv3_op(ef=4, n=24, k=3, s=1),
        mbv3_op(ef=3, n=24, k=3, s=1),
        mbv3_op(ef=3, n=40, k=3, s=2), # 3
        mbv3_op(ef=3, n=40, k=3, s=1), # 4
        mbv3_op(ef=3, n=40, k=3, s=1), # 5
        mbv3_op(ef=3, n=48, k=3, s=1),
        mbv3_op(ef=3, n=48, k=3, s=1), # add 7
        mbv3_op(ef=4, n=80, k=3, s=2), # add 8, 16x16
        mbv3_op(ef=2.5, n=80, k=3, s=1),
        mbv3_op(ef=184 / 80., n=80, k=3, s=1), # 10
        mbv3_op(ef=184 / 96., n=96, k=3, s=1),
        mbv3_op(ef=6, n=96, k=3, s=1),
        mbv3_op(ef=6, n=112, k=3, s=1), # 13
        mbv3_op(ef=6, n=112, k=3, s=1), # 14 add
        mbv3_op(ef=4, n=192, k=3, s=2), # 15
        mbv3_op(ef=4, n=192, k=3, s=1),
        mbv3_op(ef=4, n=192, k=3, s=1),
        mbv3_op(ef=5, n=320, k=3, s=1), # 18
        # op(slim.conv2d, stride=1, kernel_size=[1, 1], num_outputs=960),
        op(reduce_to_1x1, default_size=7, stride=1, padding='VALID'),
        # op(gdconv_to_1x1, default_size=7, stride=1, padding='VALID'),
        # op(slim.conv2d,
...
V3_LARGE_MINIMALISTIC_FACE_MORE6_INC = dict(
    defaults=dict(DEFAULTS),
    spec=([
        # stage 1
        op(slim.conv2d, stride=2, num_outputs=16, kernel_size=(3, 3)),  # 64x64
        mbv3_op(ef=1, n=16, k=3), # 0
        mbv3_op(ef=4, n=24, k=3, s=2), # stride2 32x32
        mbv3_op(ef=3, n=24, k=3, s=1),
        mbv3_op(ef=3, n=40, k=3, s=1),
        mbv3_op(ef=3, n=40, k=3, s=1),
        mbv3_op(ef=3, n=40, k=3, s=1), # 5
        mbv3_op(ef=3, n=40, k=3, s=1, scope='layer_added_32_1'), # add #6
        mbv3_op(ef=3, n=40, k=3, s=1, scope='layer_added_32_2'), # add #7
        mbv3_op(ef=6, n=80, k=3, s=2), # 16x16
        mbv3_op(ef=2.5, n=80, k=3, s=1),
        mbv3_op(ef=184 / 80., n=80, k=3, s=1), # 10
        mbv3_op(ef=184 / 80., n=80, k=3, s=1),
        mbv3_op(ef=6, n=80, k=3, s=1, scope='layer_added_16_1'),   # add
        mbv3_op(ef=6, n=80, k=3, s=1, scope='layer_added_16_2'),   # add
        mbv3_op(ef=6, n=80, k=3, s=1, scope='layer_added_16_3'),   # add
        mbv3_op(ef=6, n=112, k=3, s=1),
        mbv3_op(ef=6, n=112, k=3, s=1),
        mbv3_op(ef=6, n=160, k=3, s=2), # 15, 8x8
        mbv3_op(ef=6, n=160, k=3, s=1),
        mbv3_op(ef=6, n=160, k=3, s=1),
        mbv3_op(ef=6, n=160, k=3, s=1, scope='layer_added_8_1'), # 18
        op(slim.conv2d, stride=1, kernel_size=[1, 1], num_outputs=960),
        op(reduce_to_1x1, default_size=7, stride=1, padding='VALID'),
        op(slim.conv2d,
           stride=1,
...
V3_LARGE_MINIMALISTIC_FACE_MORE6 = dict(
    defaults=dict(DEFAULTS),
    spec=([
        # stage 1
        op(slim.conv2d, stride=2, num_outputs=16, kernel_size=(3, 3)),  # 64x64
        ...
        mbv3_op(ef=4, n=24, k=3, s=1),
        mbv3_op(ef=3, n=24, k=3, s=1),
        mbv3_op(ef=3, n=40, k=3, s=2), # stride2 32x32
        mbv3_op(ef=3, n=40, k=3, s=1),
        mbv3_op(ef=3, n=40, k=3, s=1), # 5
        mbv3_op(ef=3, n=40, k=3, s=1, scope='layer_added_32_1'), # add #6
        mbv3_op(ef=3, n=40, k=3, s=1, scope='layer_added_32_2'), # add #7
...
V3_LARGE_MINIMALISTIC_FACE_MORE6_RELU6 = dict(
    defaults=dict(DEFAULTS),
    spec=([
        # stage 1
        op(slim.conv2d, stride=2, num_outputs=16, kernel_size=(3, 3), activation_fn=tf.nn.relu6),  # 64x64
        mbv3_op(ef=1, n=16, k=3, act=tf.nn.relu6), # 0
        mbv3_op(ef=4, n=24, k=3, s=1, act=tf.nn.relu6),
        mbv3_op(ef=3, n=24, k=3, s=1, act=tf.nn.relu6),
        mbv3_op(ef=3, n=40, k=3, s=2, act=tf.nn.relu6), # stride2 32x32
        mbv3_op(ef=3, n=40, k=3, s=1, act=tf.nn.relu6),
        mbv3_op(ef=3, n=40, k=3, s=1, act=tf.nn.relu6), # 5
        mbv3_op(ef=3, n=40, k=3, s=1, act=tf.nn.relu6, scope='layer_added_32_1'), # add #6
        mbv3_op(ef=3, n=40, k=3, s=1, act=tf.nn.relu6, scope='layer_added_32_2'), # add #7
        mbv3_op(ef=6, n=80, k=3, s=2, act=tf.nn.relu6), # 16x16
        mbv3_op(ef=2.5, n=80, k=3, s=1, act=tf.nn.relu6),
        mbv3_op(ef=184 / 80., n=80, k=3, s=1, act=tf.nn.relu6), # 10
        mbv3_op(ef=184 / 80., n=80, k=3, s=1, act=tf.nn.relu6),
        mbv3_op(ef=6, n=80, k=3, s=1, act=tf.nn.relu6, scope='layer_added_16_1'),   # add
        mbv3_op(ef=6, n=80, k=3, s=1, act=tf.nn.relu6, scope='layer_added_16_2'),   # add
        mbv3_op(ef=6, n=80, k=3, s=1, act=tf.nn.relu6, scope='layer_added_16_3'),   # add
        mbv3_op(ef=6, n=112, k=3, s=1, act=tf.nn.relu6),
        mbv3_op(ef=6, n=112, k=3, s=1, act=tf.nn.relu6),
        mbv3_op(ef=6, n=160, k=3, s=2, act=tf.nn.relu6), # 15, 8x8
        mbv3_op(ef=6, n=160, k=3, s=1, act=tf.nn.relu6),
        mbv3_op(ef=6, n=160, k=3, s=1, act=tf.nn.relu6),
        mbv3_op(ef=6, n=160, k=3, s=1, act=tf.nn.relu6, scope='layer_added_8_1'), # 18
        op(slim.conv2d, stride=1, kernel_size=[1, 1], num_outputs=512, activation_fn=tf.nn.relu6),
        # op(flatten_to_1x1, default_size=7, stride=1, padding='VALID'),
        op(reduce_to_1x1, default_size=7, stride=1, padding='VALID'),
        # op(gdconv_to_1x1, default_size=7, stride=1, padding='VALID'),
        # op(slim.conv2d,
...
V3_LARGE_MINIMALISTIC_FACE_MORE6_4x4 = dict(
    defaults=dict(DEFAULTS),
    spec=([
        # stage 1
        op(slim.conv2d, stride=2, num_outputs=16, kernel_size=(3, 3)),  # 64x64
        ...
        mbv3_op(ef=6, n=160, k=3, s=1),
        mbv3_op(ef=6, n=160, k=3, s=1),
        mbv3_op(ef=6, n=160, k=3, s=2, scope='layer_added_8_1'), # 18
        op(slim.conv2d, stride=1, kernel_size=[1, 1], num_outputs=512),
        # op(flatten_to_1x1, default_size=7, stride=1, padding='VALID'),
        op(reduce_to_1x1, default_size=7, stride=1, padding='VALID'),
        # op(gdconv_to_1x1, default_size=7, stride=1, padding='VALID'),
...
V3_LARGE_MINIMALISTIC_FACE_MORE7_4x4 = dict(
    defaults=dict(DEFAULTS),
    spec=([
        # stage 1
        op(slim.conv2d, stride=2, num_outputs=16, kernel_size=(3, 3)),  # 64x64
        ...
        mbv3_op(ef=6, n=160, k=3, s=1),
        mbv3_op(ef=6, n=160, k=3, s=1, scope='layer_added_8_1'), # 18
        mbv3_op(ef=3, n=160, k=3, s=2, scope='layer_added_4_1'), # 18
        op(slim.conv2d, stride=1, kernel_size=[1, 1], num_outputs=512),
        # op(flatten_to_1x1, default_size=7, stride=1, padding='VALID'),
        op(reduce_to_1x1, default_size=7, stride=1, padding='VALID'),
        # op(gdconv_to_1x1, default_size=7, stride=1, padding='VALID'),
...
V3_LARGE_MINIMALISTIC_FACE_MORE6_GDCONV = dict(
    defaults=dict(DEFAULTS),
    spec=([
        # stage 1
        op(slim.conv2d, stride=2, num_outputs=16, kernel_size=(3, 3)),  # 64x64
...
V3_LARGE_MINIMALISTIC_FACE_MORE6_GDCONV_SLIM = dict(
    defaults=dict(DEFAULTS),
    spec=([
        # stage 1
        op(slim.conv2d, stride=2, num_outputs=16, kernel_size=(3, 3)),  # 64x64
        mbv3_op(ef=1, n=16, k=3, s=2), # 0
        mbv3_op(ef=4, n=24, k=3, s=1),
        mbv3_op(ef=3, n=24, k=3, s=1),
        mbv3_op(ef=3, n=40, k=3, s=1), # stride2 32x32
        mbv3_op(ef=3, n=40, k=3, s=1),
        mbv3_op(ef=3, n=40, k=3, s=1), # 5
        mbv3_op(ef=3, n=40, k=3, s=1, scope='layer_added_32_1'), # add #6
        mbv3_op(ef=3, n=40, k=3, s=1, scope='layer_added_32_2'), # add #7
...
V3_LARGE_MINIMALISTIC_FACE_MORE6_FC2_GDCONV = dict(
    defaults=dict(DEFAULTS),
    spec=([
        # stage 1
        op(slim.conv2d, stride=2, num_outputs=16, kernel_size=(3, 3)),  # 64x64
        ...
        # op(reduce_to_1x1, default_size=7, stride=1, padding='VALID'),
        op(gdconv_to_1x1, default_size=7, stride=1, padding='VALID'),
        op(slim.conv2d, stride=1, kernel_size=[1, 1], num_outputs=320, normalizer_fn=None)
    ]))

V3_LARGE_MINIMALISTIC_FACE_MORE6_GDCONV_SA = dict(
    defaults=dict(DEFAULTS),
    spec=([
        # stage 1
        op(slim.conv2d, stride=2, num_outputs=16, kernel_size=(3, 3)),  # 64x64
...
V3_LARGE_FACE_MORE6_SE = dict(
    ...
    spec=([
        ...
        mbv3_op(ef=4, n=24, k=3, s=1),
        mbv3_op(ef=3, n=24, k=3, s=1),
        mbv3_op_se(ef=3, n=40, k=3, s=2), # stride2 32x32
        mbv3_op_se(ef=3, n=40, k=3, s=1),
        mbv3_op_se(ef=3, n=40, k=3, s=1), # 5
        mbv3_op_se(ef=3, n=40, k=3, s=1, scope='layer_added_32_1'), # add #6
        mbv3_op_se(ef=3, n=40, k=3, s=1, scope='layer_added_32_2'), # add #7
        mbv3_op(ef=6, n=80, k=3, s=2, act=hard_swish), # 16x16
        mbv3_op(ef=2.5, n=80, k=3, s=1, act=hard_swish),
        mbv3_op(ef=184 / 80., n=80, k=3, s=1, act=hard_swish), # 10
        mbv3_op(ef=184 / 80., n=80, k=3, s=1, act=hard_swish),
        mbv3_op_se(ef=6, n=80, k=3, s=1, scope='layer_added_16_1', act=hard_swish),   # add
        mbv3_op_se(ef=6, n=80, k=3, s=1, scope='layer_added_16_2', act=hard_swish),   # add
        mbv3_op_se(ef=6, n=80, k=3, s=1, scope='layer_added_16_3', act=hard_swish),   # add
        mbv3_op_se(ef=6, n=112, k=3, s=1, act=hard_swish),
        mbv3_op_se(ef=6, n=112, k=3, s=1, act=hard_swish),
        mbv3_op_se(ef=6, n=160, k=3, s=2, act=hard_swish), # 15, 8x8
        mbv3_op_se(ef=6, n=160, k=3, s=1, act=hard_swish),
        mbv3_op_se(ef=6, n=160, k=3, s=1, act=hard_swish),
        mbv3_op_se(ef=6, n=160, k=3, s=1, scope='layer_added_8_1', act=hard_swish), # 18
        op(slim.conv2d, stride=1, kernel_size=[1, 1], num_outputs=512, activation_fn=hard_swish),
        op(reduce_to_1x1, default_size=7, stride=1, padding='VALID'),
    ]))

V3_LARGE_MINIMALISTIC_FACE_MORE10 = dict(
    defaults=dict(DEFAULTS),
    spec=([
        # stage 1
        op(slim.conv2d, stride=2, num_outputs=16, kernel_size=(3, 3)),  # 64x64
        ...
        mbv3_op(ef=6, n=80, k=3, s=1, scope='layer_added_16_2'),   # add
        mbv3_op(ef=6, n=80, k=3, s=1, scope='layer_added_16_3'),   # add
        mbv3_op(ef=6, n=80, k=3, s=1, scope='layer_added_16_4'),   # add
        mbv3_op(ef=6, n=80, k=3, s=1, scope='layer_added_16_5'),   # add
        mbv3_op(ef=6, n=80, k=3, s=1, scope='layer_added_16_6'),   # add
        mbv3_op(ef=6, n=80, k=3, s=1, scope='layer_added_16_7'),   # add
        mbv3_op(ef=6, n=112, k=3, s=1),
        mbv3_op(ef=6, n=112, k=3, s=1),
        mbv3_op(ef=6, n=160, k=3, s=2), # 15, 8x8
        mbv3_op(ef=6, n=160, k=3, s=1),
...
V3_SMALL_MINIMALISTIC_FACE_OP1S1 = dict(
    defaults=dict(DEFAULTS),
    spec=([
        # stage 1
        op(slim.conv2d, stride=1, num_outputs=16, kernel_size=(3, 3)),
...
V3_SMALL_MINIMALISTIC_FACE_OP1S1_GDC = dict(
    defaults=dict(DEFAULTS),
    spec=([
        # stage 1
        op(slim.conv2d, stride=1, num_outputs=16, kernel_size=(3, 3)),
...
def mobilenet_v2_arg_scope(is_training=True,
    ...
    with slim.arg_scope([slim.conv2d, slim.separable_conv2d],
                        weights_initializer=weights_init, normalizer_fn=slim.batch_norm): #tf.keras.layers.PReLU
...
def inference(images, keep_probability, phase_train=True, bottleneck_layer_size=256, weight_decay=0.0, reuse=None):
    conv_defs = V3_LARGE_FACE
    conv_defs = V3_LARGE_MINIMALISTIC_FACE_OP3S2  # 17.16MB, 1.141 G
    conv_defs = V3_LARGE_MINIMALISTIC_FACE_OP1S1  # 17.16MB, 1.234 G; dm100 7.72MB 0.541G
    # conv_defs[(slim.batch_norm,)]['is_training'] = phase_train
    ...
    with slim.arg_scope(arg_scope):
        prelogit, end_points = mobilenet(images, num_classes=bottleneck_layer_size,
                                         depth_multiplier=1.5,
                                         conv_defs=conv_defs,
                                         is_training=phase_train)
        return prelogit, end_points
