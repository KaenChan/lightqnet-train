...
def resface18(images, keep_probability,
    ...
    with tf.variable_scope('Conv1'):
        net = resface_pre(images,64,scope='Conv1_pre') # 48x48
        net = slim.repeat(net,1,resface_block,64,scope='Conv1')
    end_points['Conv1'] = net
    with tf.variable_scope('Conv2'):
        net = resface_pre(net,128,scope='Conv2_pre') # 24x24
        net = slim.repeat(net,2,resface_block,128,scope='Conv2')
    end_points['Conv2'] = net
    with tf.variable_scope('Conv3'):
        net = resface_pre(net,256,scope='Conv3_pre') # 12x12
        net = slim.repeat(net,2,resface_block,256,scope='Conv3')
    end_points['Conv3'] = net
    with tf.variable_scope('Conv4'):
        net = resface_pre(net,512,scope='Conv4_pre') # 6x6
